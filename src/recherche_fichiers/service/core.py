...
from ollama_client.embedding import embed_text
...
async def index_directory(path: str) -> int:
    ...
    files = utils.list_files_by_format(directory,
                                       SUPPORTED_FORMATS)
    if not files:
        logger.warning("Aucun fichier .txt trouvé dans %s", 
                       path)
        return 0
    ...
    logger.info("Indexation terminée : %d chunk(s) ajouté(s) depuis %s", 
                total_chunks, 
                path)
    return total_chunks
...
async def _index_file(file_path: Path) -> int:
    ...
    try:
        ...
        logger.error("Impossible de lire le fichier %s: %s", 
                     file_path, 
                     e)
        return 0

    if not content.strip():
        logger.warning("Fichier vide ignoré : %s", 
                       file_path)
        return 0
    ...
    for chunk_index, chunk_content in enumerate(chunks):
        ...
        try:
            embedding_vector = await embed_text(chunk_content)
        except Exception as e:
            logger.error("Échec de l'embedding pour le chunk: %s", 
                         e)
    ...
    logger.info("Fichier indexé : %s (%d chunk(s))", 
                file_path, 
                len(chunks))
...
async def search(query: str, 
    ...
    try:
        ...
        logger.error("Échec de l'embedding : %s",
                     e)
        return []

    raw_results = _get_store().search(query_embedding=query_embedding, 
                                      k=k)
...
if __name__ == "__main__":
    ...
    async def _demo() -> None:
        ...
        for i, result in enumerate(results, start=1):
            print(f"{i}. [{result['source_path']} - chunk {result['chunk_index']}] "
